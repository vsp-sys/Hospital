...
// Example CRUD fetchers
async function fetchBeds() { const res = await api.get('/beds'); return res.data; }
...
export default function BranchAdminPanel({
  ...
  // Removed duplicate useState for beds, appointments, prescriptions, invoices (already in props)
  const [liveBedsData, setLiveBedsData] = useState([]);
  const [liveInvoicesData, setLiveInvoicesData] = useState([]);

  // Remove useEffect fetching for beds, appointments, prescriptions, invoices if these are managed by parent
  // useEffect(() => {
